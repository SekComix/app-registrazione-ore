...
<head>
    ...
    <style>
        ...
        .material-qty { flex: 1.5; } .material-unit { flex: 2; font-size: 14px; } .material-desc { flex: 4; } /* Modifica #4: Carattere Unità di Misura */
        ...
        .photo-btn-label { display: inline-flex; align-items: center; gap: 8px; padding: 10px 15px; background-color: var(--secondary-color); color: white; border-radius: 8px; cursor: pointer; } /* Modifica #2: Preparazione per Icone */
...
<div class="container">
    ...
    <div class="card" id="form-card">
        ...
        <div class="form-group">
            ...
            <div class="action-buttons-container"> <!-- Contenitore per allineare i pulsanti -->
                <input type="file" id="photo-input-camera" accept="image/*" capture="environment">
                <label for="photo-input-camera" class="photo-btn-label">📷 Scatta Foto</label> <!-- Modifica #2: Icone Aggiunte -->
                <input type="file" id="photo-input-gallery" accept="image/*" multiple>
                <label for="photo-input-gallery" class="photo-btn-label">🖼️ Scegli da Galleria</label> <!-- Modifica #2: Icone Aggiunte -->
            </div>
    ...
    <div class="card" id="summary-card">
        <div class="summary-header">
            <h2 id="summary-title">Riepilogo Interventi</h2> <!-- Modifica #1: Testo Riepilogo -->
...
document.addEventListener('DOMContentLoaded', () => {
    const allInputs = {
        ...
        startAM: document.getElementById('start-am'), endAM: document.getElementById('end-am'),
        startPM: document.getElementById('start-pm'), endPM: document.getElementById('end-pm'),
        workDone: document.getElementById('work-done'), notes: document.getElementById('notes'),
    ...
    Object.values(allInputs).forEach(el => {
        const isTextField = el.type === 'text' || el.tagName === 'TEXTAREA' || el.type === 'date';
        if (isTextField) { el.addEventListener('input', () => el.classList.remove('error')); }
        else if (el.type === 'time') { el.addEventListener('input', () => { el.classList.remove('error'); updateTotals(); }); }
    });
    ...
    const getMaterialsData = () => {
        ...
        document.querySelectorAll('#materials-container .material-row').forEach(row => {
            ...
            if (qty && unit && desc) { materials.push({ qty, unit, desc }); }
        });
        return materials;
    };
    ...
    const handlePhotoFiles = (files) => {
        ...
        for (const file of files) {
            ...
            reader.onload = (e) => {
                ...
                currentPhotos.push({ id: photoId, file: file, dataUrl: e.target.result });
                processedImages++;
    ...
    const renderInterventions = () => {
        ...
        if (dailyInterventions.length === 0) {
            ...
            dailyInterventions.forEach(interv => {
                ...
                // Modifica #5: Logica per formattazione Orari
                let timeString = '';
                if (interv.startAM && interv.endAM) {
                    timeString += `<b>Mattino:</b> ${interv.startAM}-${interv.endAM}`;
                }
                if (interv.startPM && interv.endPM) {
                    if (timeString !== '') timeString += '<br>';
                    timeString += `<b>Pomeriggio:</b> ${interv.startPM}-${interv.endPM}`;
                }
                if (timeString === '') timeString = 'N/D';
                ...
                item.innerHTML = `
                    <div class="summary-view">
                        <div class="info">
                            ...
                            <span class="details"><b>Ore:</b> ${interv.totalHours}</span>
                        </div>
                    ...
                    <div class="full-details">
                        ...
                        <p><b>Orari:</b><br>${timeString}</p>
                        <p><b>Lavoro Svolto:</b><br>${interv.workDone || 'N/D'}</p>
    ...
    addBtn.addEventListener('click', () => {
        ...
        requiredTextFields.forEach(field => { if (!field.value.trim()) { field.classList.add('error'); textIsValid = false; } });
        if (!textIsValid) return showToast('Compila i campi evidenziati!', 'error');
        const { startAM, endAM, startPM, endPM, date } = allInputs;
        if ((startAM.value && !endAM.value) || (!startAM.value && endAM.value)) { startAM.classList.add('error'); endAM.classList.add('error'); return showToast("Orario mattino incompleto.", 'error'); }
        if ((startPM.value && !endPM.value) || (!startPM.value && endPM.value)) { startPM.classList.add('error'); endPM.classList.add('error'); return showToast("Orario pomeriggio incompleto.", 'error'); }
        const newIntervals = [];
        if (startAM.value && endAM.value) newIntervals.push({start: startAM.value, end: endAM.value});
        if (startPM.value && endPM.value) newIntervals.push({start: startPM.value, end: endPM.value});
        if (newIntervals.length > 1 && newIntervals[0].end > newIntervals[1].start) {
            allInputs.endAM.classList.add('error'); allInputs.startPM.classList.add('error');
            return showToast("L'inizio del pomeriggio non può precedere la fine del mattino.", 'error');
        }
        let existingIntervals = dailyInterventions
            .filter(i => i.id.toString() !== interventionIdInput.value && i.date === date.value)
            .flatMap(i => {
                const intervals = [];
                if (i.startAM && i.endAM) intervals.push({start: i.startAM, end: i.endAM, client: i.client});
                if (i.startPM && i.endPM) intervals.push({start: i.startPM, end: i.endPM, client: i.client});
                return intervals;
            });
        for (const newInt of newIntervals) {
            for (const oldInt of existingIntervals) {
                if (newInt.start < oldInt.end && newInt.end > oldInt.start) {
                    return showToast(`Orario ${newInt.start}-${newInt.end} si sovrappone con l'intervento per ${oldInt.client}.`, 'error', 6000);
                }
            }
        }
        ...
        const intervention = {
            ...
            date: allInputs.date.value, client: allInputs.client.value,
            startAM: allInputs.startAM.value, endAM: allInputs.endAM.value,
            startPM: allInputs.startPM.value, endPM: allInputs.endPM.value,
            totalHours: formatMinutes(totalMinutes), totalMinutes: totalMinutes,
        ...
        if (existingIndex > -1) { dailyInterventions[existingIndex] = intervention; showToast('Intervento modificato!'); }
        else { dailyInterventions.push(intervention); showToast('Intervento aggiunto!'); }
        renderInterventions();
        resetForm();
    });
    interventionsList.addEventListener('click', (e) => {
        ...
        if (action === 'delete') {
            ...
            if (interv) {
                ...
                allInputs.date.value = interv.date; allInputs.client.value = interv.client;
                ...
                allInputs.workDone.value = interv.workDone; setMaterialsData(interv.materials);
    ...
    const saveToLocalStorage = () => { localStorage.setItem('dailyInterventions', JSON.stringify(dailyInterventions)); localStorage.setItem('employeeName', allInputs.employeeName.value); };
    const loadFromLocalStorage = () => {
        const savedName = localStorage.getItem('employeeName');
        if (savedName) allInputs.employeeName.value = savedName;
        const data = localStorage.getItem('dailyInterventions');
        if (data) {
             try { dailyInterventions = JSON.parse(data); } catch(e) { dailyInterventions = []; }
        }
    };
    
    const generateFiles = async () => {
        const employeeName = allInputs.employeeName.value.trim();
        const reportDate = dailyInterventions.length > 0 ? dailyInterventions[0].date : allInputs.date.value;
        const now = new Date();
        const timeString = `${now.getHours().toString().padStart(2, '0')}-${now.getMinutes().toString().padStart(2, '0')}-${now.getSeconds().toString().padStart(2, '0')}`;
        const fileNameBase = `Report_${reportDate}_${employeeName.replace(/ /g, '_')}_${timeString}`;
        const { jsPDF } = window.jspdf;
        const doc = new jsPDF({ unit: 'pt', format: 'a4' });
        let yPos = 40;
        const pageHeight = doc.internal.pageSize.height;
        const margin = 40;
        const addHeader = (date) => {
            const formattedDateForHeader = new Date(date + 'T00:00:00').toLocaleDateString('it-IT', { day: '2-digit', month: '2-digit', year: 'numeric' });
            doc.setFontSize(22); doc.setFont(undefined, 'bold');
            doc.text(`Rapporto di Intervento Giornaliero`, doc.internal.pageSize.width / 2, yPos, { align: 'center' }); yPos += 30;
            doc.setFontSize(12); doc.setFont(undefined, 'normal');
            doc.text(`Data: ${formattedDateForHeader}`, margin, yPos);
            doc.text(`Operatore: ${employeeName}`, doc.internal.pageSize.width - margin, yPos, { align: 'right' }); yPos += 25;
            doc.setLineWidth(1.5); doc.line(margin, yPos, doc.internal.pageSize.width - margin, yPos); yPos += 20;
        };
        addHeader(reportDate);
        for (const [index, interv] of dailyInterventions.entries()) {
            const checkPageBreak = (space) => { if (yPos + space > pageHeight - margin) { doc.addPage(); yPos = margin; addHeader(interv.date); } };
            checkPageBreak(120);
            doc.setFontSize(14); doc.setFont(undefined, 'bold');
            doc.text(`Intervento #${index + 1} - Cliente: ${interv.client}`, margin, yPos); yPos += 20;
            doc.setFontSize(10); doc.setFont(undefined, 'normal');
            let timeString = (interv.startAM && interv.endAM ? `Mattino: ${interv.startAM}-${interv.endAM}` : '') + ((interv.startAM && interv.endAM && interv.startPM && interv.endPM) ? ' | ' : '') + (interv.startPM && interv.endPM ? `Pomeriggio: ${interv.startPM}-${interv.endPM}` : '');
            doc.text(`Orari: ${timeString || 'N/D'}`, margin, yPos);
            doc.text(`Ore Totali: ${interv.totalHours}`, doc.internal.pageSize.width - margin, yPos, { align: 'right' }); yPos += 20;
            const addSection = (title, content) => {
                if (content && content.trim() !== '') {
                    ...
                    checkPageBreak(lines.length * 12 + 20);
            ...
            if (index < dailyInterventions.length - 1) { doc.setLineDashPattern([5, 5], 0); doc.line(margin, yPos, doc.internal.pageSize.width - margin, yPos); doc.setLineDashPattern([], 0); yPos += 20; }
        }
        const pdfBlob = doc.output('blob');
        
        // Modifica #3: Logica Esportazione Excel
        const excelData = [];
        dailyInterventions.forEach(i => {
            if (i.materials.length > 0) {
                i.materials.forEach((m, index) => {
                    if (index === 0) {
                        // Prima riga del materiale per questo intervento: scrivo tutti i dati
                        excelData.push({
                            'Dipendente': employeeName, 'Data': i.date, 'Cliente': i.client, 
                            'Ore_Totali_Intervento': i.totalHours, 'Lavoro_Svolto': i.workDone, 'Note': i.notes,
                            'Qta_Materiale': m.qty, 'UM_Materiale': m.unit, 'Desc_Materiale': m.desc
                        });
                    } else {
                        // Righe successive per i materiali: scrivo solo i dati del materiale
                        excelData.push({
                            'Dipendente': '', 'Data': '', 'Cliente': '', 
                            'Ore_Totali_Intervento': '', 'Lavoro_Svolto': '', 'Note': '',
                            'Qta_Materiale': m.qty, 'UM_Materiale': m.unit, 'Desc_Materiale': m.desc
                        });
                    }
                });
            } else {
                // Intervento senza materiali: scrivo una sola riga
                excelData.push({
                    'Dipendente': employeeName, 'Data': i.date, 'Cliente': i.client, 
                    'Ore_Totali_Intervento': i.totalHours, 'Lavoro_Svolto': i.workDone, 'Note': i.notes,
                    'Qta_Materiale': '', 'UM_Materiale': '', 'Desc_Materiale': ''
                });
            }
        });
    ...
    saveReportBtn.addEventListener('click', async () => {
        ...
        if (!allInputs.employeeName.value.trim()) {
            allInputs.employeeName.classList.add('error');
            return showToast('Inserisci il tuo nome per salvare il report.', 'error');
        }
    ...
    sendReportBtn.addEventListener('click', () => {
        ...
        if (!employeeName) {
            allInputs.employeeName.classList.add('error');
            return showToast('Inserisci il tuo nome per inviare.', 'error');
        }

        showToast('Apro l\'app di posta...', 'info');
        const reportDate = new Date(dailyInterventions[0].date + 'T00:00:00').toLocaleDateString('it-IT');
        const subject = `Report Interventi - ${reportDate} - ${employeeName}`;
        const body = `Buongiorno,\n\nallego il report interventi del ${reportDate} compilato da ${employeeName}.\n\n(I file sono stati salvati nella cartella Download).\n\nBuona giornata.`;
    ...
    modalSavePdfBtn.addEventListener('click', () => {
        ...
        a.href = pdfUrl;
        a.download = `${generatedFileNameBase}.pdf`;
